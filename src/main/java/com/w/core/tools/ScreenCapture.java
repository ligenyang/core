...
public class ScreenCapture extends JFrame implements ActionListener {
    private JButton jButtonStart,jButtonCancel;
    private JPanel jPanel;
    private BufferedImage bufferedImage;
    private JTabbedPane jTabbedPane;//一个放置很多份图片
    private int index;//一个一直会递增的索引,用于标认图片
    private JRadioButton jRadioButtonJava,jRadioButtonSystem;//JAVA界面,系统界面
    ...
    private void initWindow(){
        jButtonStart=new JButton("开始截取");
        jButtonCancel=new JButton("退出");
        ...
        jPanel.add(jl1,BorderLayout.SOUTH);
        buttonJP.add(jButtonStart);
        buttonJP.add(jButtonCancel);
        buttonJP.setBorder(BorderFactory.createTitledBorder("公共操作区"));
        JPanel jp=new JPanel();//放置两个单选按钮的面板
        jp.add(jRadioButtonJava=new JRadioButton("java界面"));
        jp.add(jRadioButtonSystem=new JRadioButton("系统界面",true));
        jRadioButtonJava.addActionListener(this);
        jRadioButtonSystem.addActionListener(this);
        jp.setBorder(BorderFactory.createTitledBorder("界面风格"));
        ButtonGroup bg=new ButtonGroup();
        bg.add(jRadioButtonJava);
        bg.add(jRadioButtonSystem);
    ...
    private void updates(){
        ...
        if(bufferedImage!=null){
            ...
            PicPanel pic=new PicPanel(bufferedImage);
            jTabbedPane.addTab("图片"+(++index),pic);
            jTabbedPane.setSelectedComponent(pic);
            SwingUtilities.updateComponentTreeUI(jPanel); // 调整LookAndFeel（javax.swing）
        }
    }
    ...
    private void doStart(){
        try{
            ...
            Toolkit toolkit=Toolkit.getDefaultToolkit(); // AWT组件的抽象父类（java.awt）
            Dimension dimension=toolkit.getScreenSize();
            Rectangle rectangle=new Rectangle(0,0,dimension.width,dimension.height);
            BufferedImage bufferedImage=robot.createScreenCapture(rectangle);
            JFrame jFrame=new JFrame();
            Temp temp=new Temp(jFrame,bufferedImage,dimension.width,dimension.height); // 自定义的Temp类的对象
            jFrame.getContentPane().add(temp,BorderLayout.CENTER);
    ...
    public  void doSave(BufferedImage bufferedImage){
        try{
            ...
            if(i==JFileChooser.APPROVE_OPTION){
                ...
                javax.swing.filechooser.FileFilter ff=jFileChooser.getFileFilter();
    ...
    //处理关闭事件
    private void doClose(Component component){
    ...
    private class PicPanel extends JPanel implements ActionListener{
        ...
        BufferedImage bufferedImage;//得到的图片
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(source==save){
                doSave(bufferedImage);
            }else if(source==copy){
                doCopy(bufferedImage);
    ...
    private class Temp extends JPanel implements MouseListener, MouseMotionListener {
        ...
        public void paintComponent(Graphics graphics){
            graphics.drawImage(bi,0,0,width,height,this);
            ...
            graphics.fillRect(x1-2,startY-2,5,5);
            graphics.fillRect(x1-2,endY-2,5,5);
            graphics.fillRect(startX-2,y1-2,5,5);
            graphics.fillRect(endX-2,y1-2,5,5);
            graphics.fillRect(startX-2,startY-2,5,5);
            graphics.fillRect(startX-2,endY-2,5,5);
            graphics.fillRect(endX-2,startY-2,5,5);
            graphics.fillRect(endX-2,endY-2,5,5);
            ...
            if(showTip){
                ...
                graphics.fillRect(p.x,p.y,170,20);
                graphics.setColor(Color.RED);
                graphics.drawRect(p.x,p.y,170,20);
                graphics.setColor(Color.BLACK);
                graphics.drawString("请按住鼠标左键不放选择截图区",p.x,p.y+15);
            }
        }
            ...
            if(me.getClickCount()==2){
                ...
                if(select.contains(p)){
                    if(select.x+select.width<this.getWidth()&&select.y+select.height<this.getHeight()){
                        bufferedImage=bi.getSubimage(select.x,select.y,select.width,select.height);
                        jf.dispose();
                        updates();
                    }else{
